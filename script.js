...
function updateStats() {
  ...
  document.getElementById("stats").textContent = `${total} book${
    total !== 1 ? "s" : ""
  } . ${read} read`;
}
...
function clearAllErrors() {
  clearError("input-title", "error-title");
  clearError("input-author", "error-author");
  clearError("input-pages", "error-pages");
}
...
document.getElementById("input-pages").addEventListener("input", () => {
  clearError("input-pages", "error-pages");
});
...
document.getElementById("form-submit").addEventListener("click", () => {
  ...
  if (!title) {
    setError('input-title', 'error-title', "The book title must be filled!");
    valid = false;
  }

  if (!author) {
    setError('input-author', 'error-author', "The author name must be filled!");
    valid = false;
  }

  if (!pages || pages < 1) {
    setError('input-pages', 'error-pages', "Please enter a valid page count!");
    valid = false;
  }
